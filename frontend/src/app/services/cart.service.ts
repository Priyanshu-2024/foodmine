...
export class CartService {
  ...
  addToCart(foods:food): void{
    let cartItem = this.carts.items
    .find(item => item.foods.id === foods.id)
    if (cartItem) {
      return;
    }
  ...
  removeFromCart(foodId:string):void{
    this.carts.items = this.carts.items.filter(item => item.foods.id !== foodId)
    this.setToLocalStorage();
  }

  changeQuantity(foodId:string, quantity:number){
    let cartItem =  this.carts.items 
    .find(item => item.foods.id === foodId)
  ...
  private setToLocalStorage():void{
    this.carts.totalPrice = this.carts.items.reduce((prevSum, CurrentItem)=> prevSum + CurrentItem.price,0)
    this.carts.totalcount = this.carts.items.reduce((prevSum, CurrentItem)=> prevSum + CurrentItem.quantity,0)
